    ...
    while cap.isOpened():
        ...
        for _id, val in results.items():
            ...
            img = renderer.render_mesh(
                torch.from_numpy(val["verts"][frame_i2]).to(cfg.DEVICE), img
            )
...
def run_vis_on_demo_prompthmr(cfg, video, results, output_pth, smpl, vis_global=True):
    # to torch tensor
    tt = lambda x: torch.from_numpy(x).float().to(cfg.DEVICE)
    ...
    if vis_global:
        ...
        # build global camera
        # You can customize the camera view by changing these parameters:
        # - distance: how far the camera is from the subject (default: 5)
        # - position: initial camera position as (x, y, z) (default: (-5.0, 5.0, 0.0))
        # Examples:
        # - Front view: position=(0.0, 5.0, -5.0)
        # - Side view: position=(-5.0, 5.0, 0.0)
